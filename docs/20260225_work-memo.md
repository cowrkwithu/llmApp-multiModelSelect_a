...
## 15. start.sh / stop.sh 스크립트 생성

### 사용자 프롬프트
> 서비스를 시작하기 위한 start.sh 파일과 정지하기 위한 stop.sh 파일을 작성해줘.

### 생성 파일

#### `scripts/start.sh`
- Docker 서비스 (vLLM + Qdrant) 기동 + health check 대기
- Backend (FastAPI uvicorn) 백그라운드 시작
- Frontend (Next.js dev) 백그라운드 시작
- `--no-frontend` 옵션: Frontend 제외 기동
- PID 파일: `logs/backend.pid`, `logs/frontend.pid`
- 로그 파일: `logs/backend.log`, `logs/frontend.log`

#### `scripts/stop.sh`
- Frontend → Backend → Docker 순서로 정지
- PID 파일 기반 정지, fallback으로 포트 기반 탐색
- `--keep-docker` 옵션: Docker 서비스 유지하고 Backend/Frontend만 정지

### 추가 변경사항
- `.gitignore`에 `logs/` 디렉토리 추가
- `README.md`에 Quick Start/Stop 섹션 추가

---

## 16. Chat 입력창 비활성화 버그 수정

### 사용자 프롬프트
> chat 탭을 클릭하고 사용자의 프롬프트를 입력하려고 하는데 입력창이 활성화되지 않아. 입력창에 텍스트를 입력할 수 있도록 코드 수정해줘.

### 원인 분석
- `chat/page.tsx:70`에서 `disabled={useRag && !selectedCollection}` 설정
- 기본 상태: `useRag=true`, `selectedCollection=null` → 페이지 진입 시 입력창 항상 비활성화

### 수정 내용
- `disabled` prop을 `sendDisabled`로 변경 (입력창이 아닌 전송 버튼만 제한)
- `chat-input.tsx`: input 요소에서 `disabled` 속성 제거 → 항상 텍스트 입력 가능
- 컬렉션 미선택 시 placeholder 텍스트로 안내 ("Select a collection to use RAG")
- Send 버튼만 `sendDisabled`로 비활성화
...
## 17. 폼 요소 텍스트 색상 가시성 수정

### 사용자 프롬프트
> chat 창에 보여지는 입력란, 또는 리스트박스, 체크박스 등에 표시되는 글자의 컬러를 조정해줘. 현재 상태에서는 글씨가 흐려서 보이지 않아.

### 원인 분석
- `globals.css`에 `@media (prefers-color-scheme: dark)` 미디어 쿼리가 존재
- OS 다크 모드일 때 `--foreground`가 `#ededed` (밝은 색)으로 변경됨
- UI는 `bg-white`/`bg-gray-50` 등 라이트 모드로 설계 → 밝은 배경 + 밝은 글씨 = 안 보임
- 폼 요소(`<input>`, `<select>`)에 명시적 텍스트 색상 미지정 → body color 상속

### 수정 내용
1. `globals.css`: 다크 모드 미디어 쿼리 제거 (라이트 모드 전용 UI)
2. 모든 폼 요소에 명시적 `text-gray-900` 추가
3. 입력란에 `bg-white` 배경색 명시
4. placeholder에 `placeholder:text-gray-400` 적용

### 수정 파일
- `frontend/src/app/globals.css` (다크모드 제거)
- `frontend/src/app/chat/page.tsx` (체크박스 라벨)
- `frontend/src/components/chat/chat-input.tsx` (입력란)
- `frontend/src/components/models/model-selector.tsx` (select)
- `frontend/src/components/collections/collection-manager.tsx` (컬렉션명 + 입력란)
- `frontend/src/components/documents/document-list.tsx` (파일명)
- `frontend/src/components/documents/document-uploader.tsx` (파일 input)

---

## 18. Chat "Error: network error" 수정 (No-RAG 모드)

### 사용자 프롬프트
> Use RAG 체크박스를 해제하고 대화창에 메시지를 입력하여 send를 클릭하였는데 Error:network error 라는 오류가 발생했어. 원인을 확인하고 오류를 해결해줘.

### 원인 분석
두 가지 근본 원인을 발견:

**원인 1: max_tokens 초과**
- `rag_pipeline.py`에서 `max_tokens=2048` 설정
- vLLM `max_model_len=2048`이므로 프롬프트 토큰(~27) + max_tokens(2048) = 2075 > 2048
- vLLM이 400 BadRequestError 반환 → 비동기 제너레이터에서 미처리 예외 → 연결 즉시 종료

**원인 2: SSE 이중 래핑 (double-wrapping)**
- 제너레이터가 `"data: {...}\n\n"` 형식의 문자열을 직접 yield
- `sse_starlette` 2.x가 자동으로 `data:` 접두사 추가
- 결과: `data: data: {"type":"token",...}` → 프론트엔드에서 JSON 파싱 실패

### 수정 내용
1. `backend/app/services/rag_pipeline.py`:
   - `MAX_TOKENS`를 2048에서 1024로 변경 (프롬프트 토큰 여유 확보)
   - 양 함수(`query_with_rag`, `query_no_rag`)에 try/except 추가하여 에러를 SSE 이벤트로 반환
   - yield 형식을 문자열에서 dict(`{"data": json.dumps(...)}`)로 변경 (sse_starlette 2.x 호환)
2. `frontend/src/hooks/use-chat.ts`:
   - `error` 이벤트 타입 핸들링 추가 (sources/token 체크 앞에 배치)

### 검증
- `curl` 테스트: No-RAG 엔드포인트에서 `data: {"type":"token","content":"Hi"}` 정상 수신
- RAG 엔드포인트 (없는 컬렉션): `data: {"type":"error","content":"..."}` 에러 이벤트 정상 반환
- SSE 이중 래핑 문제 해결 확인

### 수정 파일
- `backend/app/services/rag_pipeline.py` (MAX_TOKENS 변경, try/except 추가, yield 형식 변경)
- `frontend/src/hooks/use-chat.ts` (error 이벤트 핸들링 추가)

---

## 19. Chat 자동 스크롤 (auto-scroll to bottom)

### 사용자 프롬프트
> chat창에서 대화가 이어지며 길어질 때 우측의 스크롤이 항상 가장 아래쪽에 위치하도록 코드를 보완해줘.

### 수정 내용
- `ChatMessages` 컴포넌트에 `useRef` + `useEffect` 기반 자동 스크롤 추가
- 메시지 목록 끝에 빈 `<div ref={bottomRef} />` 앵커 요소 배치
- `messages` 또는 `isStreaming` 변경 시 `scrollIntoView({ behavior: "smooth" })` 호출
- 스트리밍 중 토큰이 추가될 때마다 자동으로 하단 유지
